        ...
        {NAV_ITEMS.map(({ path, icon: Icon, label }) => {
          ...
          return (
            <Link
              key={path}
              to={path}
              className={`relative flex flex-col items-center gap-0.5 rounded-2xl px-4 py-2 text-[10px] font-semibold transition-all duration-200 ${
                isActive
                  ? 'text-primary'
            ...
            >
              ...
              {isActive && (
                <span className="absolute -top-0.5 left-1/2 -translate-x-1/2 h-0.5 w-8 rounded-full bg-primary" />
              )}
            </Link>
          );
        })}
